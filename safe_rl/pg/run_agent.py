...
def run_polopt_agent(env_fn,
    ...
    if env_kwargs is not None:
        assert len(env_kwargs) == 2, "specify cart and pole values!"
        env.set_values(env_kwargs[0], env_kwargs[1])
...
def run_polopt_agent_within_BO(env_fn,
                               agent=PPOAgent(),
                               actor_critic=mlp_actor_critic,
                               ac_kwargs=dict(),
                               seed=0,
    ...
    # check out the env kwargs to see the env should be deterministic or stochastic
    if env_kwargs is not None:
        try:
            with_var = env_kwargs['with_var']
        except:
            print('set whether the env should be deterministic or stochastic by setting key:with_var!')
            with_var = False
